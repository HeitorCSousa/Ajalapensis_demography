...
# Variando pelo tempo
fit1 <- openCR.fit(Ajalapensis.CHxy, type = "CJS", model = list(
  phi ~ -1 + t,
  p ~ -1 + t
))
summary(fit1)
...
pradel.fit1 <- openCR.fit(Ajalapensis.CHxy,
  type = "JSSAfCL", model = list(phi ~ t, p ~ t, f ~ t),
...
# Pradel SCR
pradel.scr.fit.0 <- openCR.fit(Ajalapensis.CHxy,
...
pradel.scr.fit.BVN0 <- openCR.fit(Ajalapensis.CHxy,
  type = "JSSAsecrfCL", mask = mesh,
  movementmodel = "BVN",
  model = list(
    phi ~ 1,
...
pradel.scr.fit.BVNzi0 <- openCR.fit(Ajalapensis.CHxy,
  type = "JSSAsecrfCL", mask = mesh,
  movementmodel = "BVNzi",
  model = list(
    phi ~ 1,
...
pradel.scr.fit.UNIzi0 <- openCR.fit(Ajalapensis.CHxy,
  type = "JSSAsecrfCL", mask = mesh,
  movementmodel = "UNIzi",
  model = list(
    phi ~ 1,
...
pradel.scr.fit.t <- openCR.fit(Ajalapensis.CHxy,
  type = "JSSAsecrfCL",
  mask = mesh,
  movementmodel = "UNIzi",
  kernelradius = 10,
...
mfull.svl <- brm(
  svl ~ plot + TSLF + t.med + t.max + t.max.abs + rh.min.abs + rh.max.abs +
...
mfull.capts <- brm(
  capts ~ plot + TSLF + t.med + t.max + t.max.abs + rh.min.abs + rh.max.abs +
    VARI.all + zentropy + tree.density + Ajalapensis_perf + Ajalapensis_ha90,
...
mfull.sex <- brm(
  sex ~ plot + TSLF + t.med + t.max + t.max.abs + rh.min.abs + rh.max.abs +
    VARI.all + zentropy + tree.density + Ajalapensis_perf + Ajalapensis_ha90,
...
loo(
  msel.svl.MeanTSLF.re, msel.svl.TSLF.re,
...
loo(
  msel.smi.MeanTSLF.re, msel.smi.TSLF.re,
...
loo(
  msel.sex.MeanTSLF.re, msel.sex.TSLF.re,
...
loo(
  msel.capts.MeanTSLF.re, msel.capts.TSLF.re,
...
ggplot(
  stack(out.inla.3.lambda.fits[, c("mean.lambda", "median.lambda", "mode.lambda")]),
...
ggplot(
  stack(log(out.inla.3.lambda.fits[, c("mean.lambda", "median.lambda", "mode.lambda")])),
  aes(x = values, fill = ind)
) +
  geom_density(alpha = 0.5, col = NA) +
