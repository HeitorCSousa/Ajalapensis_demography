...
env.vars$plot <- factor(env.vars$plot, levels = c("A1",
                                                  "A2",
                                                  "A3",
                                                  "A4"))
...
data <- aggregate(abs(des.loc$v),by=list(des.loc$sp,
                                          des.loc$SGT,
                                          des.loc$Temp,
                                          des.loc$Run,
                                          des.loc$Sex,
...
data.ctmin <- data.frame("species"=ct.sgt$Species,
                          "SGT"=ct.sgt$ID,
                          "temp"=ct.sgt$Ctmin,
...
data.ctmax <- data.frame("species"=ct.sgt$Species,
                          "SGT"=ct.sgt$ID,
                          "temp"=ct.sgt$Ctmax,
                          "run"=c(rep(NA,length(ct.sgt$Ctmin))),
                          "sex"=ct.sgt$Sex,
...
#windows(10,10)
quartz(8,8)
ggplot(data = data.complete[data.complete$species=="Ameivula_jalapensis",],
       aes(x = temp, y = Veloc)) +
...
ggplot(preddata_tpc, aes(x=temp, y=predicted)) +  geom_line(color = "blue", linewidth = 1.5) + 
  ...
  geom_point(data = data.complete[data.complete$species=="Ameivula_jalapensis",],
...
#windows(10,10)
quartz(8,8)
ggplot(data = data.complete[data.complete$species=="Tropidurus_oreadicus",],
       aes(x = temp, y = Veloc)) +
  geom_point(alpha = 0.5) +
...
ggplot(preddata_tpc, aes(x=temp, y=predicted)) +  geom_line() + 
  geom_ribbon(aes(ymin=predicted - se, ymax=predicted+se), linetype=3, alpha=.2)+
  geom_point(data = data.complete[data.complete$species=="Tropidurus_oreadicus",],
             aes(x = temp, y = Veloc), alpha = 0.5)+
  labs(y='Predicted Speed (m/s)',x='Temperature (°C)') +
...
daylength <- geosphere::daylength(lat = mean(pts.traps.SGT$lat),
...
ecophys.month$plot <- factor(ecophys.month$plot, levels = c("A1",
                                                  "A2",
                                                  "A3",
                                                  "A4"))
...
env.vars.imputed.df$trap <- paste(env.vars.imputed.df$plot, 
                                     env.vars.imputed.df$trap.int, sep = "_")
...
env.data <- left_join(env.vars.imputed.df, fire.traps.df, by=c("plot", 
                                                           "trap", 
                                                           "year", 
                                                           "month"))
head(env.data)
...
#windows(30,30)
env.data$plot <- factor(env.data$plot, levels = c("A1",
                                                  "A2",
                                                  "A3",
                                                  "A4"))
env.data$fieldtrip <- as.factor(env.data$fieldtrip)
windows(8,8)
plot.pca.env.all <- autoplot(pca.env.all, data = env.data, colour = 'plot',shape='fieldtrip',frame=T,
...
env.data$fieldtrip <- as.factor(env.data$fieldtrip)
windows(8,8)
plot.pca.env <- autoplot(pca.env, data = env.data, colour = 'plot',shape='fieldtrip',frame=T,
         loadings=T,loadings.label = TRUE,loadings.label.size = 3)
plot.pca.env
...
autoplot(pca.env, data = env.data, colour = 'plot',shape='fieldtrip',frame=T,
         loadings=T,loadings.label = TRUE,loadings.label.size = 3)
...
env.data$fieldtrip <- as.factor(env.data$fieldtrip)
windows(8,8)
plot.pca.env.ecophys <- autoplot(pca.env.ecophys, data = env.data, colour = 'plot',shape='fieldtrip',frame=T,
                         loadings=T,loadings.label = TRUE,loadings.label.size = 3)
plot.pca.env.ecophys
...
windows(height = 8, width = 10)
quartz(height = 8, width = 10)
autoplot(pca.env.ecophys, data = env.data, colour = 'plot',shape='fieldtrip',frame=T,
         loadings=T,loadings.label = TRUE,loadings.label.size = 3)
...
env.data <- env.data[,c("plot","trap","fieldtrip","severity","TSLF","t.med","t.max","t.max.abs",
