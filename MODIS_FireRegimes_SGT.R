...
SGT.fire <- mt_subset(product = "MCD64A1",
                     lat = -10.938,
                     lon = -46.756,
                     band = "Burn_Date",
                     start = "2000-01-01",
...
fire.arms.df <- data.frame(long = rep(fire.arms.wide$X,261),
...
TSLF.longdf <- data.frame(TSLF = c(unlist(c(TSLF.df[,1:220]))),
...
fire.regimes.df <- fire.arms.df %>% 
  group_by(ID,month) %>% 
  summarise(fire = sum(fire, na.rm=T),
...
fire.regimes.arms <- fire.regimes.df %>%
  group_by(ID) %>%
  summarise(freq = sum(fire,na.rm = T),
...
fire.regimes.arms$trap <- c(paste0(arms.pts.sp$campanha[1:172], arms.pts.sp$armadilha[1:172]), 
